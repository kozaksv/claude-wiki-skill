...
- Якщо wiki вже існує (orphan wiki), будь-яка операція покаже окремий repair
  gate: «Виконати `git init` тут, щоб полагодити?». Існуюча wiki не чіпається.
- Якщо wiki ще не існує, `wiki init` пропонує створити wiki разом із `git
  init`; інші операції (`lint`, `query`, `status`, `cleanup`, `split`,
  `ingest`) відмовляють і просять запустити `wiki init`.

Без підтвердження `[y]` ні `git init`, ні wiki ніколи не створюються.
