...
**Skill behavior version: 4.2.8** (`SKILL.md` frontmatter). **Install ref:** `master` by default, with `v4.2.0` available as the latest reproducible stable tag. Fresh wikis still use `wiki_version: "4.0"` because v4.1/v4.2 changed agent and installer behavior, not the on-disk schema major.
...
git-маркер (`.git/` або файл `.git`) не знайдено, `wiki init` покаже абсолютний
шлях і запитає, чи виконати там `git init`; без підтвердження wiki не
створюється і не працює. Усі інші wiki-операції (зокрема `lint`, `query`,
`status`, `cleanup`, `split`, `ingest`) у проєкті без git-метаданих відмовляють і
просять спершу запустити `wiki init`.
