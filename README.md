...
**Skill behavior version: 4.2.5** (`SKILL.md` frontmatter). **Install ref:** `master` by default, with `v4.2.0` available as the latest reproducible stable tag. Fresh wikis still use `wiki_version: "4.0"` because v4.1/v4.2 changed agent and installer behavior, not the on-disk schema major.
...
```bash
ls -la ~/.agents/skills/wiki ~/.gemini/skills/wiki 2>&1
```
...
після підтвердження і не створить другу wiki.
