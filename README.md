...
> **Ручний запуск `session-start.sh` — не перевірка без змін.** На доступній для
> запису вікі тестовий запуск у режимі `startup` також оновлює `.usage.json` і
> позначає показані одноразові нагадування як видані. Вони не повторяться на
> наступному реальному startup для того самого стану вікі чи ключа телеметрії,
> навіть якщо тестовий вивід було перенаправлено в `/dev/null`. Це не означає,
> що lint виконано або телеметрію перевірено. Для діагностики конфігурації без
> цих побічних ефектів використовуйте `hooks/doctor.sh`, як у команді вище.
...
**Старий `post_tool_use_at` ≠ вікі не читають.** Телеметрія враховує лише
підтримувані файлові інструменти й сторінки, що проходять фільтри PostToolUse.
Читання через `Bash` (`cat`, `sed` тощо) для неї невидимі — навіть якщо так
прочитано всю вікі. Timestamp у такій сесії може залишатися старим при справному
hook; це обмеження покриття телеметрії, а не доказ бездіяльності чи несправності.
`index.md`, `schema.md`, `log.md` і `log/` також не зараховуються як читання
knowledge page. Пошкоджена `.usage.json` зберігається: вона може містити
legacy protection.
