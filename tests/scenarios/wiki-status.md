...
| `b` | Run lint on the subset of 5 pages whose `last_patched_at` is oldest among `protected == false` — i.e. `[[old-service]]` (Jan 5), `[[purchase-flow]]` (Mar 15), `[[auth-service]]` (Apr 20)... (`secret-rotation-recipe` skipped because protected). Same AUTO auto-apply / DECIDE-waits machinery as `[a]`. |
