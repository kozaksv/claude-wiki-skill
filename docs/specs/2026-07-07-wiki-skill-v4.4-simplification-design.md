...
## Постскриптум (2026-07-08, після конвеєра)

Три відхилення/розширення відносно тексту вище, затверджені користувачем
після рев'ю-гейтів feature-pipeline:

1. **Passive drift notice збережено.** Рев'ю (antigravity, P1) вказало, що
   повне видалення пасивного нагадування лишає вікі без жодного сигналу про
   старіння. Затверджений компроміс: один неінтерактивний рядок
   «⚠️ Вікі: {N} пасивних дрифт-сигналів — запусти `wiki status` для деталей»,
   лише на pre-commit рефлексії і лише коли пасивні сигнали реально є. Це не
   питання (нічого не питає і не чекає відповіді) і не другий вхід у
   cleanup-flow — лише вказівник на `wiki status`.
2. **Пасивні фікси меню `wiki status` йдуть через Lint AUTO/DECIDE.** Замість
   голої правки без сліду — той самий Lint-механізм: снапшот за потреби,
   окремий revert-able коміт на кожен фікс, `відкат N`. Орієнтир користувача:
   вікі живе в git-проєкті, історія змін — через справжні коміти.
3. **Полагоджено три старі болячки контрактів** (рішення користувача
   розширити скоуп): (а) руйнівні дії cleanup-flow тепер комітяться самі
   (снапшот — лише при незакомічених змінах вікі, без порожніх
   маркер-комітів), тож `git revert HEAD` реально відкочує видалення;
   (б) lint-снапшот стейджить резолвнутий `{wiki}`-шлях, а не хардкод
   `docs/wiki/`; (в) stale-pointer repair цитує legacy-вміст старої `## Wiki`
   секції у відповіді й відкладає DECIDE-finding на наступний lint — без
   інтерактивного глухого кута під час Init/repair.
