...
- **`references/operation-wiki-status.md`**
  ...
  - **Content-check menu framing — auto-apply disclosure (review P1).** Now that `wiki status` is the sole cleanup entry point, its `[a]/[b]/[c]` choices delegate to `## Operation: Lint`, which under the Two-Tier Autonomy contract **auto-applies and commits AUTO-tier fixes without per-finding confirmation**. The current menu header at line 70 (`🔍 Хочеш зробити content-check (LLM читає сторінки, верифікує claims)?`) reads as read-only and is now misleading. Fix the framing to be honest: amend the header/subtext (line 70) and the delegation contract note (lines 89-95 + line 101) to disclose that obvious, disk-grounded fixes are auto-applied as individual reverting commits (undo with `відкат` / `відкат N`), while judgment calls (DECIDE) still wait for the user. Add one disclosure line under the `[a]/[b]/[c]` options in the output template, e.g. `(content-check може одразу застосувати очевидні виправлення окремими комітами — «відкат» їх скасує)`. This is a **disclosure-only** change: do not alter the Lint autonomy contract itself (the `>10 AUTO fixes` batch-confirm threshold, snapshot-before-fix, and page-protection all still apply). Do NOT reintroduce a `cleanup-prompt` token.
...
- **Review P1 — `wiki status` content-check is not read-only (Concern 2):** because `wiki status` is now the sole cleanup entry, its `[a]/[b]/[c]` delegation to Lint (which auto-applies + commits AUTO fixes) must be disclosed in the menu framing (Task 2). The fix is disclosure-only — the Lint autonomy contract, the `>10 AUTO` batch-confirm threshold, snapshot-before-fix, and page-protection are unchanged. If a fully read-only status is later desired, the alternative (route `[a]/[b]/[c]` to a dry-run Lint, or downgrade AUTO→DECIDE from this path) is a behavior change and belongs in its own spec, not here.
