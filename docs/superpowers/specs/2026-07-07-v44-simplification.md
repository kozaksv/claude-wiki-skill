...
- **Атомарність (знімає ризик неузгодженого CI).** Перевертання guard'а 383 і
  нові анти-return guard'и — частина implementation-коміту, НЕ окремого кроку.
  Вони МАЮТЬ landing-итися **тим самим комітом**, що видаляє токени
  (`set_skill_link` з `crystallization.md`, `🧹`, `Two entry points`,
  `writing-skills`) з `references/`. Жодна половина окремо: guard-flip без
  видалення токенів ламає CI (анти-return бачить токен); видалення без flip'а
  ламає CI (старий guard 383 вимагає наявності). Проміжного червоного стану не
  існує; покриття переноситься атомарно, а не втрачається мовчки.
