...
The wiki is not just a passive store — it is a feedback loop. After meaningful work the agent emits a **РЕФЛЕКСІЯ block** (a strict-template visible narrative) that surfaces what was learned, where it was filed, and whether anything is worth crystallizing. This is the wiki's debug-readability layer: it lets the user (and the agent itself, on the next session) see how knowledge accumulated.
...
- **Кристалізація** is mandatory. If nothing crystallized, write one of: `нічого — операція разова` / `нічого — патерн не повторюється` / `нічого — юзер відмовив раніше` / `нічого — відкладено`. The user wants to see that the question was asked. Do not invent counter-style explanations like "поточний 2/3" — there is no algorithmic threshold; see "Read the room" rule below.
