...
| `[b]` Top-5 longest unverified | Delegate to `## Operation: Lint` — content-verification flow sorted by `last_patched_at asc` (oldest first), filtered to `protected == false`. Same AUTO (snapshot + per-fix `auto-fix(wiki): #N` commit, revertible) / DECIDE (waits) split as above. |
