...
| Double-bumping `view_count`/`patch_count` when hooks are *confirmed* active | When `_hooks.post_tool_use_at` is fresh for the current session, the manual `bump_view`/`bump_patch` calls in Query/Lint/etc. are suppressed — PostToolUse already recorded the same `Read`/`Edit`. Calling both inflates counts and skews Lint's drift-risk sort. Judge suppression by a fresh `post_tool_use_at`, never by the mere fact that hooks are installed. |
| Suppressing manual bumps off the inject block alone, when PostToolUse is dead | A `WIKI INDEX (hook-injected)` block proves only that SessionStart fired — it does **not** prove `post-tool-use.sh` is running (missing `python3`, a stdin field mismatch, a write failure all leave the inject block intact while PostToolUse silently no-ops). Suppressing manual bumps on inject-presence alone produces silent, permanently-zeroed telemetry with no visible symptom. The rule is: judge by a fresh `_hooks.post_tool_use_at`, never by the inject block — see `references/telemetry.md` → Dual-signal rule. |
